...
import { View, Text, ActivityIndicator, StyleSheet, ScrollView, Image, TextInput, TouchableOpacity } from 'react-native';
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.filterRow}>
        ...
        >
          <Picker.Item label="Season (01.09.2024 - 31.08.2025)" value="season" />
        ...
        <TouchableOpacity style={styles.button} onPress={handleFilter} disabled={loading}>
          <Text style={styles.buttonText}>{loading ? 'Filtering...' : 'Filter'}</Text>
        </TouchableOpacity>
      </View>
      {error && <Text style={styles.errorText}>Error: {error}</Text>}
      <Text style={styles.title}>Group: {athleteGroupFilter === 'all' ? 'All' : athleteGroupFilter}</Text>
          ...
          <View key={ath.fincode || idx} style={styles.tableRow}>
            <View style={styles.portraitCell}>
              {ath.photo ? (
                ...
                <Image
                  source={{ uri: `https://ui-avatars.com/api/?name=${encodeURIComponent(ath.name || 'Avatar')}&background=cccccc&color=ffffff&size=50` }}
                  style={styles.portrait}
                />
              )}
            </View>
            <Text style={styles.tableCell}>{ath.name}</Text>
            <Text style={styles.tableCell}>{ath.percent != null ? ath.percent.toFixed(1) + '%' : ''}</Text>
          </View>
        ))
      )}
    </ScrollView>
